        ...
        ui.horizontal_top(|ui| {
            ui.vertical(|ui| {
                ...
                container_response.widget_info(|| {
                    WidgetInfo::labeled(WidgetType::RadioGroup, true, "Select Demo")
                });
            ...
            ui.vertical(|ui| match self.open_panel {
                Panel::Lines => {
    ...
    fn thingy<'a>(&self) -> Line<'a> {
        ...
        Line::new(
            ...
            PlotPoints::from_parametric_callback(
                move |t| ((2.0 * t * 5.0 * (0.1 * time).sin()).sin(), (3.0 * t).sin()),
                0.0..=TAU,
                256,
            ),
        )
...
impl ChartsDemo {
    ...
    fn bar_gauss(&self, ui: &mut egui::Ui) -> Response {
                ...
                .map(|(x, f)| {
                    Bar::new(x, f * 10.0)
                        ...
                        .fill(Color32::from_rgb(
                            (f * 100.0 + x * 20.0 + 128.0) as u8,
                            0,
                            (128.0 - 100.0 * f - x * 20.0) as u8,
                        ))
                })
                .collect(),
        );
        ...
        let line = Line::new(
            "Sine Wave",
            PlotPoints::from_explicit_callback(
                move |x| (x * std::f64::consts::PI / 2.0).sin() * 2.0 + 2.0,
                -3.95..=3.95,
                80,
            ),
        );
        ...
        let arrows = {
            let arrow_origins = PlotPoints::from_explicit_callback(
                move |x| (x * std::f64::consts::PI / 2.0).sin() * 2.0 + 2.0,
                -3.95..=3.95,
                80,
            );
            let arrow_tips = PlotPoints::from_explicit_callback(
                move |x| (-x * x / 2.0).exp() / (2.0 * std::f64::consts::PI).sqrt() * 10.0,
                -3.95..=3.95,
                80,
            );
            Arrows::new("arrows", arrow_origins, arrow_tips)
